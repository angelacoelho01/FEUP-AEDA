...
Car::Car(string b, string m) : brand(b), model(m), price(0.0){
}
...
FuelCar::FuelCar(string b, string m)
    : Car(b, m), tank(0), l_100km(0) {
}
...
ElectricCar::ElectricCar(string b, string m)
    : Car(b, m), battery(0), kWh_100km(0) {
}
...
HybridCar::HybridCar(string brand, string model, float price, float tank, float l_100km, float battery, float kWh_100km)
    : FuelCar(brand, model), ElectricCar(brand, model) {
...
float HybridCar::range() const {
    float tank = getTank(), l_100km = getL_100km();
    float battery = getBattery(), kWh_100km = getKWh_100km();
    return (tank*100)/l_100km + (battery*100)/kWh_100km;
}
