...
bool User::operator<(const User& u1) const {
	//TODO:
	unsigned numRead = titlesRead.size() + (titleReading != pair<string, string>()? 1 : 0);
	unsigned  numRead1 = u1.getReadings().size() + (u1.getReading() != pair<string, string>()? 1 : 0);
	return numRead < numRead1;
}
