...
void Client::addVisiting(string address, string postalCode, string typology, string price) {
	//if visiting is blank, set it
		if(get<0>(this->propertyListing) == "" && get<1>(this->propertyListing) == "" && get<2>(this->propertyListing) == "" && get<3>(this->propertyListing) == "") {
...
bool Client::operator<(const Client& c1) const {
	//TODO:
    int priceLastVisited = stoi(get<3>(propertyListing));
    int priceLastVisited1 = stoi(get<3>(c1.getVisiting()));
	return priceLastVisited < priceLastVisited1;
}
