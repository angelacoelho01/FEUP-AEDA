...
void Doctor::moveToFront(unsigned codP1) {
	queue<Patient> aux;
	Patient* urgentPatient = nullptr;
	while(!patients.empty()) {
        if (patients.front().getCode() == codP1) urgentPatient = &patients.front();
        else aux.push(patients.front());
        patients.pop();
    }
    if(urgentPatient != nullptr) patients.push(*urgentPatient);
    while(!aux.empty()){
